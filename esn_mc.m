%% LOAD DATASET
dataset = csvread('dataset/mc100.csv', 19);
...
for sigma = sigmas
    ...
    %% ESN TRAINING
    err_pool_tr = zeros(1, esn_pool);
    err_pool_val = zeros(1, esn_pool);
    ...
    for i=1:esn_pool
        ...
        err_pool_tr(i) = memory_capacity(output_tr, y_tr, ntransient);
        
        % validation
        [ output_val, err_val ] = esn_score(X_val, y_val, W_in, W_hat, W_out, ip_a, ip_b, ntransient);
        err_pool_val(i) = memory_capacity(output_val, y_val, ntransient);
    end
...
for i = 1:esn_pool
    ...
    err_pool_tr(end + 1) = memory_capacity(y_tr_pred, [y_tr y_val], ntransient);
    y_tr_pred_pool = [y_tr_pred_pool; y_tr_pred];
    
    % test
    [ y_ts_pred, err_ts ] = esn_score(X_ts, y_ts, W_in, W_hat, W_out, ip_a, ip_b, ntransient);
    err_pool_ts(end + 1) = memory_capacity(y_ts_pred, y_ts, ntransient);
